#include "GameInspectorSections.h"
...
/// Inspector の「ゲーム固有欄」本体。選択中エンティティを受け取って描画する。
void DrawGameComponents(IImGuiEditable* selected) {
#ifdef _DEBUG
    if (!selected) return;
...
void Register() {
    ImGuiManager::Instance().AddInspectorSection("Game Components", &DrawGameComponents);
}
