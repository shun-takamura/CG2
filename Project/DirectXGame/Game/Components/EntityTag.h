...
enum class EntityTag : int {
	None = 0,
	Player,            // 自機
	PlayerBullet,      // 自機の弾（飛び道具のヒット判定）
	PlayerMelee,       // 自機の近接攻撃（コリジョン＋ダメージのみ）
	Enemy,             // 敵全般
...
inline std::string_view GetTagName(EntityTag t) {
	...
	case EntityTag::PlayerMelee:      return "PlayerMelee";
...
inline void GetTagColor(EntityTag t, float& r, float& g, float& b, float& a) {
	...
	case EntityTag::PlayerMelee:      r = 0.65f; g = 0.80f; b = 1.00f; break;
