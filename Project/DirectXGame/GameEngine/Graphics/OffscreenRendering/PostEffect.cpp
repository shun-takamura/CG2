...
void PostEffect::CreateBasePsoDesc()
{
	...
	// 頂点シェーダー（エフェクト共通）
	IDxcBlob* vsBlob = dxCore_->LoadShaderBlob(L"Resources/Shaders/PostEffect/Common/PostProcess.VS.hlsl", L"vs_6_0");
	assert(vsBlob);
	basePsoDesc_.VS = { vsBlob->GetBufferPointer(), vsBlob->GetBufferSize() };

	// コピー用パイプライン
	{
		IDxcBlob* copyVsBlob = dxCore_->LoadShaderBlob(L"Resources/Shaders/PostEffect/Common/CopyImage.VS.hlsl", L"vs_6_0");
		assert(copyVsBlob);
		IDxcBlob* copyPsBlob = dxCore_->LoadShaderBlob(L"Resources/Shaders/PostEffect/Common/CopyImage.PS.hlsl", L"ps_6_0");
		assert(copyPsBlob);
