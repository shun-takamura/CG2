...
PixelShaderOutput main(VertexShaderOutput input)
{
    ...
    float vignette = correct.x * correct.y * 16.0f; //vignetteScale;
        
    // カーブ調整
    vignette = pow(vignette, 0.8f/*vignettePower*/);
        
    // 0〜1にクランプ
    vignette = saturate(vignette);
        
    //float factor = lerp(1.0f, vignette, vignetteIntensity);
    output.color.rgb *= vignette;//factor;
