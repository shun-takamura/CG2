...
import claude_chat  # スマホからの設計対話(/ask /plan /plan_save)

# /refactor で使う定型指示（規約クリーンアップ。実際の規約は ai_edit のシステムプロンプト側）
REFACTOR_INSTRUCTION = "コーディング規約に従って変数名・冗長コードを整理してリファクタリングして"

# 多重起動ガード：127.0.0.1 の固定ポートを掴み、掴めなければ既に起動中とみなして終了する。
# ロックファイルと違い、プロセスが死ねばOSがポートを解放するので取り残し(stale lock)が起きない。
_SINGLE_INSTANCE_PORT = 50515  # 他のアプリと被らない適当な未使用ポート
_lock_socket = None  # GC でソケットが閉じてロックが外れないようグローバルで保持
...
def _ensure_single_instance():
    ...
    try:
        ...
        print("J.A.R.V.I.S. Bot は既に起動しています。多重起動はできません。")
        print("（既存を止めてから起動し直してください）")
        sys.exit(1)
...
async def live_start_cmd(interaction: discord.Interaction):
    ...
    try:
        # SUNDAY稼働中は既にゲーム窓が開いているので、新規起動せず間借りする（二重起動防止）。
        # start_live 側でも窓の有無を見て二重起動を防ぐ（別プロセスのSUNDAY/手動起動にも効く）。
        sunday_running = bool(_sunday_thread and _sunday_thread.is_alive())
        result = await asyncio.to_thread(live_control.start_live, not sunday_running)
        mode = "🆕 新規にゲームを起動して配信" if result.get("launched") else "🔗 既存のゲーム窓に接続して配信"
        await interaction.followup.send(
            f"{interaction.user.mention} 🔴 配信開始！（{mode}） ボイスチャンネルでゲーム画面を視聴できます。"
        )
    except Exception as e:
...
async def live_stop_cmd(interaction: discord.Interaction):
    await interaction.response.defer()
    try:
        # SUNDAY稼働中はゲーム窓を閉じない（閉じるとSUNDAYがクラッシュと誤検知するため）
        sunday_running = bool(_sunday_thread and _sunday_thread.is_alive())
        await asyncio.to_thread(live_control.stop_live, sunday_running)
        tail = "ゲームはSUNDAYが使用中なので開いたままです。" if sunday_running else "ゲームを閉じました。"
        await interaction.followup.send(
            f"{interaction.user.mention} ⏹️ 配信終了。{tail}"
        )
    except Exception as e:
...
# ===== スマホ単独で設計まで完結する対話系（/ask /plan /plan_save） =====

# 進行中の設計対話セッションID（/plan で継続、/plan_new で破棄）
_plan_session_id = None


# Discordの2000字制限に合わせて返す。長ければ .md ファイルで添付する
async def _send_reply(interaction, header, body):
    if len(header) + len(body) + 1 <= 1900:
        await interaction.followup.send(f"{header}\n{body}")
    else:
        f = discord.File(io.BytesIO(body.encode("utf-8")), filename="claude.md")
        await interaction.followup.send(f"{header}（長いのでファイルで返します）", file=f)


# /ask コマンド：読み取り専用の単発相談（コードは編集しない）
@tree.command(name="ask", description="Claudeに相談（読み取り専用・コード編集なし）")
@app_commands.describe(question="質問・相談内容")
async def ask_cmd(interaction: discord.Interaction, question: str):
    await interaction.response.defer()
    try:
        text = await asyncio.to_thread(claude_chat.ask, question, REPO_DIR)
        await _send_reply(interaction, f"{interaction.user.mention} 💬", text)
    except Exception as e:
        await interaction.followup.send(f"{interaction.user.mention} ❌ ask 失敗: {e}")


# /plan コマンド：設計対話（会話継続・編集なし）
@tree.command(name="plan", description="Claudeと設計対話（会話継続・編集なし）")
@app_commands.describe(instruction="設計の指示・追記（前回の続きから対話する）")
async def plan_cmd(interaction: discord.Interaction, instruction: str):
    global _plan_session_id
    await interaction.response.defer()
    try:
        text, sid = await asyncio.to_thread(
            claude_chat.plan, instruction, REPO_DIR, _plan_session_id
        )
        if sid:
            _plan_session_id = sid  # 次回はこのセッションを継続
        await _send_reply(interaction, f"{interaction.user.mention} 🧭 plan", text)
    except Exception as e:
        await interaction.followup.send(f"{interaction.user.mention} ❌ plan 失敗: {e}")


# /plan_new コマンド：設計対話をリセットして新しく始める
@tree.command(name="plan_new", description="設計対話をリセットして新規に始める")
async def plan_new_cmd(interaction: discord.Interaction):
    global _plan_session_id
    _plan_session_id = None
    await interaction.response.send_message(
        f"{interaction.user.mention} 🆕 設計対話をリセットしました。次の /plan から新規セッションです。")


# /plan_save コマンド：設計対話の結論を Chat/<name>.md に保存
@tree.command(name="plan_save", description="設計対話の結論を Chat/<名前>.md に保存する")
@app_commands.describe(name="保存名（拡張子なし）")
async def plan_save_cmd(interaction: discord.Interaction, name: str):
    await interaction.response.defer()
    if not _plan_session_id:
        await interaction.followup.send(
            f"{interaction.user.mention} ⚠️ まだ /plan の対話がありません。先に /plan で設計してください。")
        return
    try:
        dest, _ = await asyncio.to_thread(
            claude_chat.save_plan, name, REPO_DIR, _plan_session_id
        )
        rel = dest.relative_to(REPO_DIR)
        await interaction.followup.send(
            f"{interaction.user.mention} 💾 設計を `{rel}` に保存しました。\n"
            f"実装は `/prompt` で「{rel} の通りに実装して」と送ってください。")
    except Exception as e:
        await interaction.followup.send(f"{interaction.user.mention} ❌ plan_save 失敗: {e}")
