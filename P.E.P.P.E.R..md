...
## 9. 実装着手メモ（2026-06-14 設計確定 / 2026-06-16 並行制約を解除）

当初は別PCの「エフェクトエディタのゲームからの独立」と並行する前提で衝突回避（ImGui後回し等）を組んでいたが、**エフェクトエディタ側のAIエフェクト生成が困難でその作業を進めない判断となり、並行制約は解除**（2026-06-16）。よって **計画書 §7 の通常順（ImGuiを含む 1→7）でそのまま実装してよい**。ImGuiManager / ParticleManager のmerge衝突懸念も気にしなくてよい。

> 調査で判明した恒久的な事実（並行と無関係に有効）: エンジンのライブラリ化リファクタで **エンジン汎用コードは `EngineCore.vcxproj` に集約済み**。SessionLogger / RandomGenerator / CrashHandler / ReplaySystem / DirectXCore / 各Manager / Effect系すべて EngineCore 所属。**PEPPER の新規ファイルも `EngineCore.vcxproj`（と `.filters`）に追加する**（`CG2_0_1.vcxproj` ではない）。`Framework` は Effect系を一切含まず、`Game/Scene/SceneManager` も EffectManager 非参照＝PEPPER計装はクリーン。

### 確定事項
- **ブランチ**: master から `Engine_PEPPER_00` を新規に切る。
- **有効化**: 専用マクロ **`USE_PEPPER`**。**Debug / Development 構成のみ** `PreprocessorDefinitions` に `USE_PEPPER` を追加、**Release には入れない**。`PEPPER_SCOPE`/`PEPPER_GPU_SCOPE` は `#ifdef USE_PEPPER` で本体、未定義時は `((void)0)` に空展開（オーバーヘッドゼロ）。SUNDAY の `_DEBUG` とは独立に制御。
- **配置**: 新規フォルダ **`GameEngine/Profiling/`** に `Profiler.{h,cpp}` ＋ マクロヘッダ（`PEPPER_SCOPE`/`PEPPER_GPU_SCOPE` 定義。`__FILE__`/`__LINE__` をトークン連結で一意名に）。**`EngineCore.vcxproj` と `EngineCore.vcxproj.filters`** に `<ClCompile>`/`<ClInclude>` 追記（→ユーザーへ報告）。
- **SessionLogger 連携**: `SessionLogger::Category` に **`Profile`** を1個追加（`Session` の後、`kCount` の前）→ `profile.log`。レベルは `Trace`（高頻度。`USE_PEPPER` ビルドでのみ書き出し）。`SessionLogger.h` の enum へ1行＋`SessionLogger.cpp` のファイル名テーブルへ1行。
- **計装対象（v1）**: `Framework::Update/Draw` → `SceneManager::Update/Draw` → 主要マネージャー（Sprite / Object3D / Particle / Skybox / Text / Line）の `Update/Draw`。各所は `PEPPER_SCOPE("名前")` を関数先頭に1行入れるだけ。Effect系（EffectManager等）の計装はv1スコープ外＝必要になれば後で追加。
...
5. ImGui リアルタイム表示（`ImGuiManager` に PEPPER ウィンドウ追加。`USE_IMGUI` ガード）
